...
class MixStructureBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.conv3_1 = nn.Conv2d(dim, 2 * dim, kernel_size=1) # 1*1
        self.conv3_2 = nn.Conv2d(2 * dim, dim, kernel_size=1) # 1*1

        self.conv1 = nn.Conv2d(dim, dim, kernel_size=1)
        self.conv2 = nn.Conv2d(dim, dim, kernel_size=5, padding=2, padding_mode='reflect')
        self.conv3_19 = nn.Conv2d(2 * dim, 2 * dim, kernel_size=7, padding=9, groups=2 * dim, dilation=3, padding_mode='reflect')
        self.conv3_13 = nn.Conv2d(2 * dim, 2 * dim, kernel_size=5, padding=6, groups=2 * dim, dilation=3, padding_mode='reflect')
        self.conv3_7 = nn.Conv2d(2 * dim, 2 * dim, kernel_size=3, padding=3, groups=2 * dim, dilation=3, padding_mode='reflect')

        # Simple Pixel Attention
        self.Wv = nn.Sequential(
            nn.Conv2d(2 * dim, 2 * dim, 1),
            nn.Conv2d(2 * dim, 2 * dim, kernel_size=3, padding=3 // 2, groups=dim, padding_mode='reflect')
        )
        ...
        self.ca = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(2 * dim, 2 * dim, 1, padding=0, bias=True),
            nn.GELU(),
            # nn.ReLU(True),
            nn.Conv2d(2 * dim, 2 * dim, 1, padding=0, bias=True),
            nn.Sigmoid()
        ...
        self.pa = nn.Sequential(
            nn.Conv2d(2 * dim, 2 * dim // 8, 1, padding=0, bias=True),
            nn.GELU(),
            # nn.ReLU(True),
            nn.Conv2d(2 * dim // 8, 1, 1, padding=0, bias=True),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        identity = x
        x = self.conv3_1(self.norm1(x))
        # x = self.conv1(x) # 1*1 时候去掉
        # x = self.conv2(x) # 1*1 时候去掉
        x1_1 = self.conv3_19(x)
        x2_1 = self.conv3_13(x)
        x3_1 = self.conv3_7(x)
        x = torch.cat([self.conv3_2(x1_1), self.conv3_2(x2_1), self.conv3_2(x3_1)], dim=1) # 1*1
        # x = torch.cat([self.conv3_19(x), self.conv3_13(x), self.conv3_7(x)], dim=1) # 原始
        x = self.mlp(x)
        x = identity + x

        identity = x
        # x = self.norm2(x)
        x = self.conv3_1(self.norm2(x))
        # x = torch.cat([self.Wv(x) * self.Wg(x), self.ca(x) * x, self.pa(x) * x], dim=1)
        x1_2 = self.Wv(x) * self.Wg(x)
        x2_2 = self.ca(x) * x
        x3_2 = self.pa(x) * x
        x = torch.cat([self.conv3_2(x1_2), self.conv3_2(x2_2), self.conv3_2(x3_2)], dim=1) # 1*1
        x = self.mlp2(x)
        x = identity + x
        return x
